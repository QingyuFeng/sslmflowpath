...
int main(int argc,char **argv)
{
   ...
   char lzpointfile[MAXLN], lzareafile[MAXLN];
   int err,nmain, i;
   ...
   if(argc < 2)
	...
	while(argc > i)
	{
		...
		else if (strcmp(argv[i], "-lzps") == 0)
		{
			i++;
			if (argc > i)
			{
				strcpy(lzpointfile, argv[i]);
				i++;
			}
			else goto errexit;
		}
	...
	if(argc == 2)
	{
		...
		nameadd(lzpointfile, argv[1], "lzpointsub.txt");
		nameadd(lzareafile, argv[1], "lzareasub.txt");
	}

    if(err= lorenzSub(pfile,distfile,wsfile, lufile, elevfile, slpfile, lzpointfile, lzareafile) != 0)
	...
	errexit:
	   ...
	   printf("<lzpointfile> is the lorenz point text output file.\n");
	   ...
	   printf("lzps   text file containing lorenz points (output)\n");
	   printf("lzas   text file containing lorenz area (output)\n");
       exit(0);
} 
